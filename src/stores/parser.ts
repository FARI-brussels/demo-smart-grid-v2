...
export function parseData(data: CMSData): CMSDataParsed {
  ...
  if (images_sdg && images_sdg.data)
    images_sdg.data.forEach(({ attributes }) =>
      item.media.sdg.push(import.meta.env.VITE_API_URL + attributes.url)
    )

  if (caroussel && caroussel.data)
    caroussel.data.forEach(({ attributes }) =>
      item.media.logos.push(import.meta.env.VITE_API_URL + attributes.url)
    )

  if (video && video.data)
    item.media.video = import.meta.env.VITE_API_URL + video?.data?.attributes?.url
